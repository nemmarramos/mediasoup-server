...
export class WssRoom extends EnhancedEventEmitter implements IRoom {
  ...
  public async consume(data: IPeerTransport): Promise<Object> {
    try {
      ...
      const userMediaClient = data.toConsumePeerId
        ? this.clients.get(data.toConsumePeerId).media
        : this.host.media;
  ...
  public async connectWebRTCTransport(data: IProducerConnectorTransport) {
    try {
      ...
      if (data.type === 'producer') {
        await user.media.producerTransport.connect({
          dtlsParameters: data.dtlsParameters,
        });
      }

      if (data.type === 'consumer') {
        await user.media.consumerTransport.connect({
          dtlsParameters: data.dtlsParameters,
        });
      }
      return true;
  ...
  public async sendGift(gift: IGift, peerId: string): Promise<void> {
    const user = this.clients.get(peerId);
    this.broadcastAll('onGiftReceived', { gift, user: user.profile });
  }
